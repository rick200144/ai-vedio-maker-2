...
If the site deploys but Vercel shows `NOT_FOUND`, the most common cause is that the project is pointed at the wrong **Root Directory**. In Vercel, open **Project Settings → General → Root Directory** and make sure it points to this Next.js app folder (the one that contains `package.json`, `app/`, and `next.config.ts`). Then redeploy.

This package also includes a `vercel.json` file with `framework: "nextjs"` so Vercel can detect the project type more reliably.
